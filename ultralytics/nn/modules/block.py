...
from ..backbone.inceptionnext import InceptionDWConv2d, ConvMlp
...
class Bottleneck_IDWC(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5, mlp_ratio=1.2, area=1):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = InceptionDWConv2d(c_)
        # self.cv1 = ABlock(c_, c_ // 32, mlp_ratio, area)
        # self.cv2 = InceptionDWConv2d(c_)
        # self.cv2 = ABlock(c_, c_ // 32, mlp_ratio, area)
        self.cv2 = CPCA(c_)
...
class A2C2f_IDWC(nn.Module):
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True, k=3):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.gamma = nn.Parameter(0.01 * torch.ones(c2), requires_grad=True) if a2 and residual else None
        self.m = nn.ModuleList(
            nn.Sequential(Bottleneck_IDWC(c_, c_, shortcut, g, k=(k, k), e=1.0,  mlp_ratio=mlp_ratio, area=area))
            if a2
            else C3k(c_, c_, 2, shortcut, g)
            for _ in range(n)
        )
...
class A2C2f_IDWB(nn.Module):
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True, k=3):
        ...
        self.gamma = nn.Parameter(0.01 * torch.ones(c2), requires_grad=True) if a2 and residual else None
        self.m = nn.ModuleList(
            nn.Sequential(*(Bottleneck_IDWC(c_, c_)for _ in range(2)))
            if a2
            else C3k(c_, c_, 2, shortcut, g)
            for _ in range(n)
        )
